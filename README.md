...
- **全盘搜文件名**：文件名过滤框输入关键词，毫秒级返回**全盘**命中；结果**双击在资源树中定位并展开所在路径**（文件夹则直接展开）；文件名 / 路径两列宽度可拖动（可选引擎：在 exe 同目录放 `es.exe` 即启用，voidtools 官网免费下载；无它则提示不可用，其余功能不受影响）
...
- **专注模式**：`F8` 淡化非当前段落
- **显示比例**：右下角缩放滑杆（50%~200%），`Ctrl+滚轮` 缩放、`Ctrl+0` 复位——只缩正文内容区，设置重启后记住
- 启动即聚焦编辑区，打开就能直接打字
...
| 专注模式     | `F8` 淡化其他段落                                                                                  |
| 缩放         | 右下角滑杆 /`Ctrl+滚轮` / `Ctrl+0` 复位（50%~200%，只缩正文）                                    |
